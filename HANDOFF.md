...
- **타깃 안의 항 유계화**: **MDAC**(Iwaki 2025, arXiv:2502.03854) — critic 타깃의 α log π 항만 tanh/clip(x/10)로 유계화, actor·auto-α 그대로; "log π 집중 → α 증가 → α log π 폭발 → Q 훼손" 진단 일치; 느리게 포화하는 bound가 공격적 bound보다 낫다(Fig. 4c). Munchausen RL(Vieillard et al., NeurIPS 2020) [τ ln π]_{l₀}^{0}, l₀ = −1 클리핑(이산·현재 행동 항·고정 τ). SCQ(Wu et al. 2026, arXiv:2609.12749; O2O·실로봇) −log π를 sigmoid 유계 점수로(actor·critic 공유 α). You 2023(arXiv:2305.11831, 미심사) 타깃에서 α·H₀를 빼 보너스를 α(H − H₀)로 중심화.
...
- **문구**: "우리가 아는 한, actor의 자동 α는 유지한 채 critic TD 타깃 안에서만 α_c = min(α, cap)(0 < cap < ∞)으로 제한하는 중간 형태를 연속 제어 또는 offline-to-online에서 시험한 연구는 없다. 검증된 선행은 이 family의 두 끝점(cap = ∞: SAC v2, cap = 0: RLPD/Cal-QL 코드/Asad/DSAC-E)이거나 다른 레버(항 자체의 유계화 MDAC·Munchausen·SCQ, 보상 스케일 SAC v1, 목표 엔트로피)다." 인용 통제군: RLPD(hard backup 선례), Asad(계수 분리 선례), SAC v1(보상 스케일 arm 원전), MDAC(온건한 bound 우선의 사전 근거). finder만 읽고 검증 안 된 18편(SigEnt-SAC, XQL, DrQ-v2, EXPO, LPS, SAC Flow, TES-SAC 등)은 전문의 별도 표 — 본문 주장에 쓰지 않는다.
