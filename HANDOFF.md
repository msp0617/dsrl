...
> **현재 상태(2026-09-17): §27 + [results/2026-09-17/README.md](results/2026-09-17/README.md)를 먼저 읽을 것 — §26 실험 결과(16 run). 확정은 Q3 하나(γ 0.99이면 auto-α가 1.7에 머묾 → α 폭주의 원천은 γ; 성능은 나쁨, 진단용). Q1a(9/6 tent12 배치 재현) 미결(127k +0.078, 동역학·중반 하강은 재현), 같은 밤 α₀ 효과 127k +0.010 → §25의 α₀ 결론 철회(시사 이하), Q1c cap 고유 효과 n=5 127k 기준 없음(+0.028), Q2 cfix1 = cap1(1.0은 시점이 아니라 수준 문제; 형식 미결). 탐색적: critic 온도 0.3 arm(cap03 n=5·cfix03)은 online 20–70k에서 0.55 고원, 무제한 α arm은 0.33–0.42로 처짐(post-hoc 창 평균 W: cap03 − tent12i +0.080 ± 0.025 5/5, cfix03 +0.076 3/3; tent12r − tent12 −0.004) — 127k 단일점(seed SD 0.03–0.13)이 아니라 W + 127k를 지표로 사전 등록해야 함. 채팅용 [HANDOFF_CHAT_2026-09-17.md](HANDOFF_CHAT_2026-09-17.md).**
> **9/16 낮 상태: §26 — 다음 실험 사전 등록(α₀ 교란 정리 `colab/vm_square_a0.ipynb` 7 run, critic 온도 고정 + γ 진단 `colab/vm_square_cfix.ipynb` 9 run; 코드 `train.critic_alpha_fixed`), 아직 안 돌림. 결과는 §25 + [results/2026-09-16/README.md](results/2026-09-16/README.md)를 먼저 읽을 것 — Square 전이 실험(§24 설계) 결과: 보상 정규화 기각, critic 타깃 보너스 상한 0.3은 α 되먹임을 끊고 Square 무데모 최고(초기 AUC 0.490, 127k 0.522)이나 대조군 `square_tent12i`(초기 α 0.3, 127k 0.485)가 tent12 대비 이득의 대부분을 설명 → cap 고유 효과는 시사; cap 1.0은 해로움; `can_prefill_fixa015` 129k 0.930. 채팅용 요약 [HANDOFF_CHAT_2026-09-16.md](HANDOFF_CHAT_2026-09-16.md). 문헌 §24.1.**
> **9/15 상태: §24 — 설계·사전 등록·노트북(`colab/vm_square_rs.ipynb`, `colab/vm_square_cap.ipynb`).**
> **9/12 밤 상태: §21–23과 [results/2026-09-12/README.md](results/2026-09-12/README.md).**
> 온라인 본 실험 154 run(9/8 121 + 9/12 hq/prefill_t12i 6 + prefill_alpha 6 + 9/15–16 Square 전이·prefill_fixa015 21), 오프라인 artifact 32/32.
...
새 세션 첫 메시지: **"HANDOFF.md §21–23과 results/2026-09-12/README.md 붙여넣고, §23 '다음(권고)'부터 이어서"** (§2의 '진행 중'은 9/4 상태).
...
- **다음 실험(권고, 코드 변경 없음)** — 질문: "데모 배치에서 초기 α/엔트로피 수준을 0.3/13–16으로 붙들면 5k·10–15k dip이 모두 없어지는가".
  ...
  - 보조 arm `can_prefill_t12i_a015_s{1,2,3}`: `… train.ent_coef=auto_0.15 train.target_ent=12` — 과도기 뒤의 α에서 시작(undershoot 제거)해 "과도 vs 정상상태"를 가름. **노트북: `colab/vm_prefill_alpha.ipynb`**(vm2_new 판형, 7번 셀이 6개 launch, 9번 keepalive 자동 반납, 10번 zip).
...
## 23. 9/12(토) 밤 — vm_prefill_alpha 결과: 고정 α 0.3은 데모 배치의 두 dip을 seed 평균에서 없애지만 fixalpha_03 대비 이득이 없고 데모의 후반 이득을 잃는다; α 0.15 시작은 과도를 못 없앤다. 그림·표 `results/2026-09-12/prefill_alpha/`, 설명 `results/2026-09-12/README.md`

- **실행**: 6 run 모두 `[done] 150000`(마지막 평가 env 129,152 = Can 150k run의 정상 종료 모양, 18 evals, ckpt b@150000). 원본은 `csv_bundle (6).zip`(Drive, 9/12 21:47 KST; 140 entries = **133 scientific run** + smoke/utility/pretrain) → Windows 데스크톱 `logs/bundle_20260912/logs/`(git-ignored). 수치는 `scripts/plot_results.py`(9/7 정의; 노트북 10번 셀의 `--axes`) 값이며 원본 CSV에서 **독립 3회 재계산**해 전부 일치(268/268). 진단 행은 평가보다 208(5k)~832(20k) env step 앞의 train_log 행. `~/Downloads/logs`는 어느 컴퓨터든 옛 번들이 섞여 있으니 쓰지 않는다.
- **주 지표(사전 등록 §22: online 5,008 seed 평균 ≥ 0.405 + seed-matched 차이)**
  - `can_prefill_fixa03`: **0.493 ± 0.099**(0.420/0.370/0.690) ≥ 0.405 → **글자대로 통과**(미달 seed는 s2 하나; s1 0.420은 기준 위 0.015로 100-episode 평가 SE 0.050 안). seed-matched: fixalpha_03 s1–3(0.443) 대비 **+0.050 ± 0.227**(1/3, paired p 0.85), mix_prefill(0.780) 대비 −0.287 ± 0.064(0/3), prefill_t12i(0.097) 대비 +0.397 ± 0.084(3/3). 단 0.405는 fixalpha_03·baseline의 3-seed 부분집합도 각각 8/10이 넘고, 같은 run의 step-0(0.537)보다 낮은 기준이라 "통과"는 **prefill_t12i형 5k dip이 seed 평균에서 없다**는 뜻이지 fixalpha_03과의 구분도, "dip 없음"도 아니다(seed 평균 곡선은 5–20k 내내 0.42–0.50으로 step-0 아래, 25k에 0.783).
  - `can_prefill_t12i_a015`: **0.283 ± 0.169**(0.100/0.620/0.130) → **실패**.
- **보조 지표**
  - 10k/15k seed 평균: fixa03 0.500/0.480(10–15k 평균 0.490 ± 0.103; mix_prefill 0.093/0.303 대비 +0.292 ± 0.056, 3/3 → **mix_prefill형 10–15k dip 없음**; fixalpha_03 대비 −0.083 ± 0.097, 20k −0.187 ± 0.052, 0/3). 격자 최저 0.420@20k(§21의 옛 "최저" 기준으로도 0.015 차 통과). a015 0.540/0.887(10–15k 평균 fixalpha_03 대비 +0.140 ± 0.016, 3/3).
  - 초기 AUC: fixa03 **0.654 ± 0.026**(참조 0.71 미달; fixalpha_03 n=5 0.656 ± 0.016, seed-matched −0.025 ± 0.030) → 데모가 초기 AUC를 안 올림(§22 기대 +0.003 그대로). a015 **0.774 ± 0.039**(참조 0.78 도달; prefill_t12i 0.776 ± 0.007, seed-matched −0.002 ± 0.034).
  - 129k / 104k·129k 평균: fixa03 **0.763 ± 0.059 / 0.736 ± 0.030**(기대 ~0.85 미달; fixalpha_03 n=5 0.739 ± 0.023 / 0.748 ± 0.020, seed-matched +0.038 ± 0.091 / −0.004 ± 0.047; prefill_t12i 대비 −0.107 ± 0.032 / **−0.131 ± 0.014**, 0/3). a015 **0.892 ± 0.029 / 0.862 ± 0.010**(129k 단일 평가로는 Can 최고 표본평균이나 prefill_t12i 0.870 ± 0.073·mix_prefill 0.858 ± 0.049와 SE 안: seed-matched +0.022 ± 0.049 / +0.033 ± 0.063; last-3 `final`로는 0.839 ± 0.032로 둘보다 뒤 → "최고"는 시점을 붙여서만).
  - T80: fixa03 25,040/60,096/25,040; a015 **15,024 ×3**(prefill_t12i 20,032 ×3; a015 − prefill_t12i 10k +0.170 ± 0.015, 15k +0.340 ± 0.026, 모두 3/3).
- **판정**: "둘 다(5k ≥ 0.405 ∧ 129k ≥ 0.80)"는 **두 arm 다 아니다**. fixa03 = §21 용어로 **'부분'**(초반 통과, 후반 < 0.80, 데모가 있어도 후반 이득 없음); a015 = **실패**(5k) + 후반 상위. 0.53은 §21 Q1(hq) 기준이므로 여기엔 쓰지 않는다.
- **해석 1 — 가장 강한 후반 결과**: **고정 α 0.3에서는 데모가 성공률에 거의 무익하다**(초기 AUC −0.025 ± 0.030, online 5–20k −0.076 ± 0.029, 104k/129k −0.004 ± 0.047; 모두 seed-matched vs fixalpha_03). §22가 기대한 "데모 후반 이득 +0.133 ± 0.016 견고"는 auto-α 데모 arm에서 얻은 기대였고 **고정 α에서는 재현되지 않았다**. 무데모 참조 수준이 같은데(fixalpha_03 s1–3 0.725 vs tent12i 0.697, 0.4 SE) 데모 arm의 후반(104k/129k)은 고정 0.3 0.736 ± 0.030 vs 목표 12 0.867 ± 0.036(prefill_t12i) / 0.862 ± 0.010(a015), Welch +0.13(2.8–4.0 SE). 목표 0(mix_prefill − baseline +0.322 ± 0.063, iql_prefill − iql +0.297 ± 0.053, calql_prefill − calql +0.256 ± 0.008)까지 이어 "엔트로피를 낮출수록 단조 증가" 사다리로 쓰지 않는다 — 인접 단은 1–2 SE, 목표 0 단의 크기는 낮은 참조(baseline s1–3 0.473)의 몫(auto-α 데모 arm 후반은 전부 0.83–0.89). **인과("엔트로피가 떨어지도록 허용해서")로도 쓰지 않는다**: 고정 0.3 arm은 α 수준(0.300 vs 0.14–0.17), critic 타깃의 엔트로피 보너스 규모(아래 해석 4), 배치 구성이 함께 다르다. 두 수준 패턴("고정 0.3만 후반 이득 0, auto-α 데모 arm은 +0.19~+0.32")까지가 **시사**.
- **해석 2 — 5k**: **auto-α를 0.15에서 시작해도 시작 과도는 사라지지 않았다**. 초기값은 로그에 없고(첫 행 online 1,200에 이미 0.102; prefill_t12i 0.196, mix_prefill 0.643 — 초기 0.15/0.30/1.0은 코드 경로와 첫 행 비율로만 확인), online 2,400에 최저 0.075/0.078/0.075, 5k 직전 행(env 28,816) 0.121/0.123/0.118, 0.15 회복은 7,200. H(90% 데모 배치 기준) 16.4–16.9 → 5k 행 11.96/9.77/11.07(중간 골 10.5/10.8/8.6), |μ| 0.23–0.28 → 0.49–0.54. 이유: α 손실 `-(log α·(log p + target))`의 기울기가 H − 목표이고(o2o_utils.py:162) Adam이 step당 ≈ lr(3e-4)만큼 log α를 움직이므로 **H₀ 16–19 > 12인 한 초기값과 무관하게 α는 먼저 떨어지고, 초기값은 과도의 위상만 옮긴다**(a015 H 골 2.4–3.6k, prefill_t12i 골 4.8k = 5k 평가 직전). 그래서 a015는 5k 상태가 prefill_t12i와 거의 같아(α 0.12 vs 0.10, H 9.8–12.0 vs 8.1–10.3) 5k dip 2/3, 설계 목적("과도 vs 정상상태")은 못 이뤘고, 대신 골이 먼저 지나가서 10k/15k 회복이 빠르다(T80 15k). "초기값은 무관"은 고원(AUC·129k)에만 해당.
- **해석 3 — §22 "수준 규칙" 수정**: "평가 직전 α≈0.1, H≈7–10, |μ|≈0.6 → 9/9 ≤ 0.34"에 예외 **a015 s2**(α 0.123, H 9.8, |μ| 0.54 → 0.620) → 9/10(엄격 6/7); tent12i s2(α 0.133, H 10.6 → 0.740)도 경계 예외, tent12i s3는 5k 행 H 12.5로 규칙 범위 밖(예외 아님). 진단이 있는 Can 73 run의 5k 점에서 이 상태는 **규칙이 아니라 상관**: Spearman |μ| −0.58, H +0.55, Q_W +0.56; dip(< 0.405) 비율 |μ| ≥ 0.5에서 26/36, < 0.5에서 14/37. **데모가 저α 상태의 dip 비용을 키운다고 쓸 수 없다**(상태를 맞추면 데모 4/5 vs 무데모 9/10, |μ| ≥ 0.5에서 5/7 vs 21/29; tent12i만 참조로 고른 5/6 vs 1/3은 착시). α≈0.1도 필요조건이 아니다(tent12i_hq α 0.18–0.21, H 8–10에서 3/3 dip; hardq α 0.18–0.19도 3/3). 5–20k 전체로 넓히면 규칙은 깨진다(저α·저H 53쌍 중 27쌍 ≥ 0.405; H < 5 붕괴 상태 41쌍 중 22쌍 ≥ 0.405, mix_prefill은 H≈0을 유지한 채 0.86까지 회복) → §22의 "붕괴 상태가 원인" 금지는 그대로.
- **해석 4 — Q_W는 critic 진단이 아니라 엔트로피 보너스다**: Can 전 run에서 `qw_mean ≈ 56.8·(α·H) − 48.2`(R² 0.933, 후반 138행), 5–20k에서도 39.9·αH − 29.5(R² 0.71; critic이 α보다 ~5k 뒤짐), 평가 시작 상태에서 `q_start − mc_return ≈ 76.5·αH + 41`(R² 0.945); 계수는 Σ_{t≤74} 0.99^t = 51.9와 맞는다(**Can discount는 0.99**, cfg `train.discount`; 0.999는 Square). 따라서 α 스케줄별 Q_W 궤적(5k/10k/15k/20k 행 seed 평균: mix_prefill +113/+20/−30/−57, prefill_fixa03 +56/+105/+115/+100, a015 −30/+1/+15/+24, prefill_t12i −13/−7/−11/−10)은 **구성상** 다른 것이고, **Q_W 부호 반전을 dip의 표지로 쓰지 않는다**: + → − 반전은 목표 0 soft-critic arm 전부에 있고(baseline s4/s5, iql_prefill, mix_fixed/linear, rs_2), 반전 없이도 dip(hardq·tent12i_hq: 보너스 0, Q_W 항상 음; rs_025: 항상 양), 반전이 있어도 dip 없음(calql_prefill, a015 − → +). mix_prefill의 10k dip(0.093)은 αH < 1(online 7,200, 3/3)·H 붕괴(17.5 → 2.1@9.6k)와 겹치고 Q_W 영점 통과(12,000, 3/3)보다 **앞선다** — §22의 "α 감쇠 + critic 타깃 재척도 전이 구간"에서 재척도는 α 감쇠의 결과다.
- **단일 seed 진단(주장 아님)**: prefill_fixa03 s3의 10–15k 0.280/0.300(합산 0.290 ± 0.032, p=0.5 이항으로 6e-6 → 잡음 아님)은 α 0.300, H 13.4–15.0, |μ| 0.34–0.44, Q_W +104.6 → +121.0으로 dip 없는 s1/s2와 진단이 같고, 같은 seed는 무데모(fixalpha_03 s3)도 5k/10k 0.19/0.32다. s3는 Can 모든 그룹에서 step-0 최저(0.27–0.46)인 어려운 평가 세트 → seed 고유 초기 불안정으로 읽는다. "prefill의 dip은 엔트로피와 무관"의 근거로 쓰지 않는다.
- **진단 읽을 때 주의**: prefill arm의 train_log 진단(H, |μ|, Q_W, α 손실)은 `offline_p` 0.896(5k) → 0.85(20k) → 0.624(129k)의 **데모 상태 배치**에서 계산된다(o2o_utils `train()`이 replay_data.observations로 계산). 무데모 arm과의 수준 비교(예: "데모가 H를 3.6 nat 올린다")는 다른 상태 분포 위의 비교이고, **목표 엔트로피 제어기도 데모 가중 H를 조절**하므로 같은 목표 12에서 데모 arm은 α 0.14–0.17, tent12i는 0.20–0.29(후반 0.31–0.44)에 머문다 — 해석 1의 교란 중 하나.
...
- **실행(두 VM, 같은 밤, 150k, 5k 격자, `variant=baseline offline_mix.mode=none load_offline_data=False`)**
  ...
  - cap이 물리는 시점 예상: tent12의 α는 첫 bin(online 0–5k)부터 평균 0.40 > 0.3이라 tent12 궤적에서는 cap 0.3이 처음부터 끝까지 물린다; tent12i(초기 0.3)는 H₀ > 12인 동안 α가 먼저 내려갔다가 오르므로 처음 몇 k step은 안 물리고 online ~5–10k(tent12가 0.39 → 0.62로 오르는 구간)부터 물릴 것으로 예상; cap 1.0 → online ~15–20k(env 47–52k, tent12 bin 0.94 → 1.33)부터. 실제 시점은 `critic_ent_coef == cap`으로 읽는다. 42k(online 10k) 시점의 critic 온도: tent12 ≈ 0.4–0.6 / cap03 0.3 / hq 0. 보너스 상한 0.3 × 12 ≈ 3.6/step → Q_W ≈ 수백(Σγ^t ≈ 95 × 3.6 + 수익; tent12 23,000).
...
### 24.1 문헌(9/15–16, 6갈래 검색 → 우선 9편 원문 검증; 전문 `results/2026-09-16/literature_critic_alpha_cap.md`)

- **critic 타깃 온도를 actor와 분리**: Asad, Babanezhad, Vaswani 2025(arXiv:2509.09838, 이산 SAC/Atari) — 타깃 `r + γ[q − ζ ln π]`의 ζ를 actor의 자동 τ와 분리, ζ = τ 결합이 성능 저하의 주원인, ζ = 0 채택(ζ ∈ {0, τ}만). Zhan et al. TECRL/DSAC-E 2025(arXiv:2511.11592, MuJoCo) — "온도 가중 엔트로피 항이 Q 타깃을 비정상으로 만든다"는 같은 진단, reward critic은 α 없는 hard backup + 별도 α-free 엔트로피 critic. 둘 다 유한 cap·O2O·확산 정책 없음.
- **타깃 안의 항 유계화**: **MDAC**(Iwaki 2025, arXiv:2502.03854) — critic 타깃의 α log π 항만 tanh/clip(x/10)로 유계화, actor·auto-α 그대로; "log π 집중 → α 증가 → α log π 폭발 → Q 훼손" 진단 일치; 느리게 포화하는(거의 안 물리는) bound clip(x/10)·tanh(x/10)가 자주 물리는 clip(x,−1,1)보다 낫고 sign(x)가 최악(Fig. 4c–d). MDAC의 "공격적"은 값이 아니라 **자주 물리는 bound**(clip(x,−1,1)는 현재 (s,a)에서 클리핑이 잦아 상대 정보를 잃고, clip(x/10)은 거의 안 물린다; Fig. 4c–d, 9/16 원문 확인)를 뜻한다 — 우리 결과는 92% 행에서 물리는 cap 0.3이 79%에서 물리는 cap 1.0보다 나으므로 **MDAC과 방향이 같다고 쓸 수 없다**(비교 단위도 다르다: per-sample α·log π 곱 vs 계수 α). §26의 '물림 충격' 가설이 맞으면 '부드러운 bound가 낫다'와만 닿는다. Munchausen RL(Vieillard et al., NeurIPS 2020) [τ ln π]_{l₀}^{0}, l₀ = −1 클리핑(이산·현재 행동 항·고정 τ). SCQ(Wu et al. 2026, arXiv:2609.12749; O2O·실로봇) −log π를 sigmoid 유계 점수로(actor·critic 공유 α). You 2023(arXiv:2305.11831, 미심사) 타깃에서 α·H₀를 빼 보너스를 α(H − H₀)로 중심화.
- **hard backup + soft actor(= cap 0)**: RLPD(Ball et al. ICML 2023, arXiv:2302.02948) `backup_entropy` 플래그를 도메인별 선택으로 두고 절제 — AntMaze-large·Adroit·humanoid에서 "엔트로피 백업이 항상 더 나빴다"; Cal-QL(JaxCQL)·SERL/HIL-SERL 코드 기본값 `backup_entropy=False`(9/16 raw 파일에서 직접 확인; rlpd 기본 True); LP-DS(Simsir & Oguz ICML 2026) robomimic Can/Square에서 엔트로피 없는 hard backup + latent trust region(soft/hard 절제 없음).
- **보상 스케일 = 1/온도**: SAC v1(Haarnoja et al. ICML 2018) 원전; SAC v2의 자동 온도는 상한 없는 쌍대 변수. 전역 log α clamp(−16, 2)는 ElegantRL(actor·critic 공통), TorchRL `max_alpha` 옵션; SB3·CleanRL·jaxrl·rlpd에는 clamp 없음(직접 확인).
- **문구**: "우리가 아는 한, actor의 자동 α는 유지한 채 critic TD 타깃 안에서만 α_c = min(α, cap)(0 < cap < ∞)으로 제한하는 중간 형태를 연속 제어 또는 offline-to-online에서 시험한 연구는 없다. 검증된 선행은 이 family의 두 끝점(cap = ∞: SAC v2, cap = 0: RLPD/Cal-QL 코드/Asad/DSAC-E)이거나 다른 레버(항 자체의 유계화 MDAC·Munchausen·SCQ, 보상 스케일 SAC v1, 목표 엔트로피)다." 인용 통제군: RLPD(hard backup 선례), Asad(계수 분리 선례), SAC v1(보상 스케일 arm 원전), MDAC(타깃 내 항 유계화 선례·진단 일치; cap 값의 근거로는 쓰지 않음). finder만 읽고 검증 안 된 18편(SigEnt-SAC, XQL, DrQ-v2, EXPO, LPS, SAC Flow, TES-SAC 등)은 전문의 별도 표 — 본문 주장에 쓰지 않는다.

## 25. 9/16(수) — Square 전이 실험(§24) + `can_prefill_fixa015` 결과. 숫자·판정 `results/2026-09-16/README.md`, 채팅용 `HANDOFF_CHAT_2026-09-16.md`

- **실행**: 21 run 전부 정상 종료(9/16 01:07 번들, 154 run). 지표는 이 PC(Python 없음)에서 9/8 정의를 PowerShell로 재구현(`results/2026-09-16/analyze_20260916.ps1`)해 계산, 기존 그룹으로 검증(tent12 0.404/0.326@50,080/0.369, hq − tent12 +0.093 ± 0.020 등 전부 일치). 그림은 데스크톱 `.venv`에서.
- **갈래 A 기각**: `square_tent12i_rs02` 42k 0.370, 127k 0.358 ± 0.011, **α 16.9–19.5**(예측 "< 1" 실패), Q_W 25k(전부 보너스); `square_fixa015_rs02` 127k 0.085(고정 α의 Square 엔트로피 붕괴, fixalpha_03 경로 그대로: H 12 → −3.8, |μ| 0.87). → **α 폭주의 원천은 보상 Q 스케일이 아니라 보너스 자체의 되먹임.**
- **갈래 B**: `square_tent12i_cap03` — 되먹임이 끊김(α ≤ 2.0, Q_W ≤ 1.6k; cap onset online 9.6–11.2k, 행 92%), 초기 AUC **0.490**(9/8 `square_td` 0.492 ± 0.020와 SE 안 동률, 그 외 Square arm 위 — "전 Square arm 최고"는 9/16 교정; tent12 s1–3 대비 +0.062 ± 0.032 3/3, hq 대비 +0.112 ± 0.011 3/3 → 관건 지표 통과), 102k 0.562, 127k 0.522 ± 0.068(hq 0.490 동급, tent12 대비 +0.125 ± 0.038 3/3), late 0.542 ± 0.027. 글자대로는 판정표 3행(42k 0.333 < 0.404)이나 AUC 절은 반대 → "후반 hq급 + 초반 AUC 최고 + 42k 단일점 dip 잔존". `square_tent12i_cap1` **해로움**(127k 0.240 ± 0.025; tent12 대비 −0.157 ± 0.024, 대조군 대비 −0.245 ± 0.041, 0/3) → 용량 비단조, 원인 열림.
- **대조군이 결정적**: `square_tent12i`(cap·rs 없음, 초기 α 0.3) 127k **0.485 ± 0.021**, late 0.465 — tent12(초기 1.0) 대비 late +0.098 ± 0.009(3/3), 127k +0.088 ± 0.035(3/3)인데 후반 진단(α 18–20, Q_W 29.5k, H 12)은 tent12와 같다. 즉 §19.8·9/8 README §6의 "Square 목표 12는 후반에 무너진다"는 **α₀ = 1.0에서의 관찰**이고, hq 이득(+0.093)과 α₀ 효과(+0.088)는 크기가 같고 tent12i ≈ hq @127k(−0.005 ± 0.015)다 — 단 **가산성은 말할 수 없다**(hq는 α₀ 1.0, tent12i는 α₀ 0.3이라 같은 α₀에서 hq 유무를 비교한 것이 아니다; `square_tent12i_hq`가 있어야 한다, 9/16 채팅 교정). cap 고유 효과 = 대조군 대비 102k +0.117 ± 0.039(3/3), late +0.077 ± 0.030(3/3), 127k +0.037 ± 0.072(1/3) → **시사**. **Q_W 크기는 후반을 설명하지 않는다**(30k → 0.485, 4k → 0.240, 1.5k → 0.522, −75 → 0.490).
- **42k dip 미해결**: tent12i 계열 전부 0.33–0.39(< π_dp 0.494); cap은 42k 직후에 물리므로 42k에 작용 불가.
- **Can 회귀 `can_tent12i_cap03`**: 사전 기준 통과 → **해치지 않음**(5k 0.410 ≥ 0.405, seed 2/3는 아래; tent12i 대비 AUC +0.048 ± 0.055 2/3, 129k +0.075 ± 0.045 2/3, late +0.093 ± 0.060 3/3). cap이 34–81% 행에서 물렸는데도 129k **0.772 ± 0.018** = 무데모 Can 최고(fixalpha_03 대비 +0.047 ± 0.020 3/3; 초기 AUC −0.037 ± 0.015 0/3). hq(tent12i_hq)와는 전 지표 3/3 차이.
- **`can_prefill_fixa015`(§23 판별)**: 5,008 seed 평균 **0.473**(0.31/0.75/0.36; 5k bin α 0.150/H 11.5/|μ| 0.50 → 조건 밴드 안) → 5k dip은 **과도**(수준 아님; prefill_t12i 0.097·a015 0.283 대비 +0.377 ± 0.129·+0.190 ± 0.031, 3/3); 104k/129k **0.898** ≥ 0.80 → 데모 후반 이득은 제어기 없이 나오고 고정 0.3의 무익함은 **α 수준** 문제(prefill_fixa03 대비 late +0.163 ± 0.042 3/3). 129k 0.930 ± 0.021 = Can 최고 표본평균(a015·prefill_t12i와 SE 안). 엔트로피 9.3–11.6 유지. 단 seed 2/3가 5k < 0.405, s1 10k 0.27.
- **쓰면 안 되는 문장**: `results/2026-09-16/README.md` §5(cap 해결 확정·Q 폭주 원인·보상 정규화·cap 값 무관·Can no-op·42k 해결·tent12 붕괴 Square 고유·α 0.15가 답·fixa015 dip 제거/Can 최고 등).
- **다음(권고, 채팅에서 확정)**: (1) α₀ 교란 정리 — `square_tent12i_s{4,5}` + `square_tent12i_cap03_s{4,5}` + `square_tent12_s{1,2,3}` 같은 배치 재실행; (2) cap 용량 0.15·0.5 또는 tanh bound; (3) `square_prefill_t12i_cap03`. 하지 않는 것: 보상 정규화 확장, β 스케줄, 목표 엔트로피 스케줄, α 사다리, 300k, LP-DS.

## 26. 9/16(수) — α₀ 교란 정리 + cap 1.0 퍼즐(critic 온도 고정 `train.critic_alpha_fixed`, 신규) + γ 진단. 노트북 `colab/vm_square_a0.ipynb`(VM SA, 7 run, 코드 변경 없음) · `colab/vm_square_cfix.ipynb`(VM SB, 9 run, 코드 변경 있음). **아직 안 돌림.**

- **채팅 분석(9/16 낮, `HANDOFF_CHAT_2026-09-16.md` 위에서)의 결론과 우선순위**: §24 가설 4단계 중 (1) "α ∝ Q 스케일" → **기각**(rs02: 보상 ×0.2에도 α 17–20), (2)(3) 보너스 누적 → 확정, (4) "Q 폭주 → 순위 상실" → **기각**(Q_W 30k → 0.485, 4k → 0.240, 1.5k → 0.522). cap이 작동은 하지만 왜 작동하는지는 열림. 가장 큰 새 사실은 α₀ 교란(§25). 우선순위: (1) α₀ 교란·배치 차이 정리, (2) cap1 퍼즐 — cap이 값에 비단조(∞ 0.485 > 1.0 0.240 < 0.3 0.522)인데 값 말고 **물리는 시점**이 다르다(cap03 online ≈ 10k에 Q_W ≈ 85, cap1 online ≈ 24–26k에 Q_W ≈ 1,000; cap1 곡선은 물린 뒤에 떨어진다: seed 평균 62k 0.553 → 97k 0.313 → 127k 0.240 — 62k·97k 값은 9/15 밤 keepalive 상태 줄 파싱이라 `results/2026-09-16/` 표에는 없음) → "물림 충격"(커지는 보너스에 맞춰 학습된 critic이 고정 보너스 타깃으로 급변해 재적합하는 과도기) vs "수준 효과"(1.0이 과함)를 **critic 온도를 처음부터 고정**해 분리, (3) γ 진단(고리 이득 ∝ H/(1−γ): Can 100배·Square 1,000배), (4) `square_prefill_t12i_cap03`은 1–2 뒤로. cap 용량 곡선(0.15·0.5)은 값과 시점이 얽혀 곡선을 그려도 해석이 안 되므로 뒤로.
- **문구 교정 반영**(§24.1·§25·`results/2026-09-16/README.md` §4·§5·문헌 파일·`HANDOFF_CHAT` §5·§7·§8): (i) MDAC(arXiv:2502.03854)의 "공격적 bound"는 값이 아니라 **자주 물리는 bound**(clip(x,−1,1) vs clip(x/10); Fig. 4c–d, 원문 확인) → 우리 cap03(92% 물림) > cap1(79%)과 "방향 일치"라고 쓸 수 없어 삭제, cap 값의 사전 근거로도 인용하지 않음; (ii) "hq 이득과 α₀ 효과는 더해지지 않는다"는 hq(α₀ 1.0)와 tent12i(α₀ 0.3)를 비교한 것이라 **같은 α₀에서의 가산성이 아님** → "tent12i ≈ hq @127k"까지로 철회(`square_tent12i_hq` 필요); (iii) 금지 문장에 "cap 0.3이 Square 최고 arm"(127k는 mix_prefill 0.622가 위; 초기 AUC도 9/8 `square_td` 0.492 ± 0.020와 SE 안 동률이라 "전 Square arm 최고"가 아님) 추가.
- **구현(코드 변경 1개)**: `train.critic_alpha_fixed` c > 0 → 타깃 `r + γ(Q̄ − c·log π′)`를 **첫 업데이트부터** 상수 c로(물림 시점 없음), actor 손실 `α·log π − Q_W`는 auto-α 그대로. `o2o_utils.critic_ent_coef(ent_coef, cap, fixed)`(`th.full_like`), `DSRLResumable.critic_alpha_fixed`(기본 −1 = 끔), `critic_temperature_settings(cfg)`가 cap과 **상호 배타**(둘 다 > 0이면 ValueError; `train_dsrl.py`의 초기 rule-out 블록에서 env·정책을 만들기 전에 호출, `build_agent`에서 다시 대입), `config_fingerprint`에 `critic_alpha_fixed`와 **`discount`**(γ 0.999 checkpoint가 0.99로 resume되지 않게; 옛 checkpoint는 키가 없으므로 그대로 resume — 테스트로 고정) 추가. train_log `critic_ent_coef` = c. `cfg/robomimic/dsrl_{can,square}.yaml`에 `critic_alpha_fixed: -1`. `plot_results.py` DEFAULT_AXES `square_a0`·`square_cfix`, `gce/runs_example.txt`, `O2O.md` 표. 테스트: `test_offline_mix.py` +3(fixed 기본값·fingerprint·옛 fingerprint resume·다른 규칙 거부, 상수 산술·cap 배타, discount fingerprint) = 25, `test_notebook_launches.py` +2 + 셀 매직 구조 검사(`%bash` 오타를 잡는다; 실제로 첫 빌드에서 잡힘) = 12, `test_resume_state.py` 12 — **데스크톱 .venv에서 전부 통과**(torch 불필요). VM SB 6번 셀: 같은 테스트 + 스모크(고정 actor α 1.0에 `critic_alpha_fixed=2.0` → `critic_ent_coef` 전부 2.0 = **min이 아니라 상수**임을 증명; −1 → `ent_coef`와 동일) + cap·fixed 동시 지정이 거부되는지.
- **실행 순서**: ① 이 커밋을 `origin/o2o`에 push(두 노트북의 3번 셀이 `critic_alpha_fixed`가 HEAD에 있는지 확인하므로 push 전에는 못 돈다) → ② 두 노트북을 GitHub `o2o`에서 열어 각각 0→9(1번 뒤 런타임 재시작 → 2번부터; 6번이 실패하면 7번 금지) → ③ 끝나면 10번 셀(CPU 런타임)로 zip → 로컬 `logs/bundle_<날짜>/` → `python scripts/plot_results.py --logs <폴더>/logs --out results/<날짜>/square_a0_cfix --axes "square_a0=square_baseline,square_tent12,square_tent12r,square_tent12_hq,square_tent12i,square_tent12i_cap03;square_cfix=square_tent12,square_tent12i,square_tent12i_cap03,square_tent12i_cap1,square_tent12i_cfix03,square_tent12i_cfix1,square_tent12i_g099"`. 두 VM 같은 밤, 150k, 5k 격자, `variant=baseline offline_mix.mode=none load_offline_data=False`; 16 run ≈ G4 두 대 × 5–6 h ≈ 크레딧 100 안팎.
- **VM SA `colab/vm_square_a0.ipynb`**(vm3_new 판형, Square 자산만, 7 run ≈ 125 GB): `square_tent12r_s{1,2,3}` = `train.target_ent=12`(9/6 `square_tent12`와 같은 override — §19.5 기록 기준, auto-α SB3 초기 1.0; 9/6 launch 셀 원문은 저장소에 없으므로 6번 셀이 9/6 run의 `run_state.json` fingerprint를 찍어 대조한다; **exp_id를 바꾼 이유**: 같은 이름이면 Drive의 완료된 checkpoint를 resume해 곧바로 [done]), `square_tent12i_s{4,5}`, `square_tent12i_cap03_s{4,5}`.
  - **Q1a 배치 재현**: seed-matched `tent12r − tent12`(s1–3; 9/6 s1–3 127k 0.345/0.455/0.390, late 0.385/0.348/0.368)의 127k 차와 late 차(각각). 둘 다 |평균 차| ≤ 0.05 이고 어느 쪽도 (3/3 ∧ |t| > 4.3)가 아님 → **"배치 차이 없음(시사)"**(9/6 참조 그대로; n=3 동등성 주장이라 '시사'; 0.05 ≈ tent12 s1–3 127k SE 0.032의 1.5배). 어느 한쪽이라도 3/3 ∧ |t| > 4.3, 또는 |평균 차| > 0.10 → **"배치 효과"**: 9/6 tent12를 참조에서 빼고 tent12r로 대체, §25의 α₀ 결론은 "미확정"으로 강등. 그 외 → 미결(기술만). **결과에 따라**: 배치 효과 또는 미결이면 Q1b의 "α₀ 효과 확정" 라벨은 쓸 수 없고 Q1b는 같은 밤 짝 `tent12i s1–3 − tent12r s1–3`(§23 규칙, n=3)과 기술적 s4–5 비교만 보고; Q1c는 영향 없음(cap03·tent12i의 s1–3은 둘 다 9/15, s4–5는 둘 다 오늘); Q2는 아래 의존 조항. 진단 재현(기술만, 판정에 안 씀): online 110k 5k bin(110,000–114,999)의 α ≥ 10, Q_W ≥ 10k(tent12 20.1/29.6k).
  - **Q1b α₀ 효과 n=5**(Q1a = 배치 차이 없음일 때만): seed-matched `tent12i − tent12`(s1–5; s4–5 짝은 9/6 tent12 s4–5 127k 0.320/0.335 vs 오늘 tent12i s4–5) 127k·late. **df=4 규칙(|t| > 2.78 ∧ ≥ 4/5 같은 방향)** 통과(127k 또는 late, 나머지 같은 부호) → "α₀ 효과 확정"; 아니면 "시사"(현재 n=3 그대로). 보조: 같은 밤 짝 `tent12i s4–5` vs `tent12r`(n=2 vs 3, 기술만).
  - **Q1c cap 고유 효과 n=5**: seed-matched `cap03 − tent12i`(s1–5) 127k·late·102k. df=4 규칙 통과(127k 또는 late, 나머지 같은 부호) → "cap 고유 효과"; 통과 못 해도 127k·late 둘 다 ≥ 4/5 같은 방향 ∧ 평균 > 0 → "시사"; 그 외 → "없음". 검정력: paired SE ≈ 0.05 → 0.15 이상만 가른다.
  - 42k는 단일점(100 ep)이라 기술만.
- **VM SB `colab/vm_square_cfix.ipynb`**(코드 변경 있음 → 3번 셀이 `critic_alpha_fixed` 커밋 확인, 5번이 스모크용 Can 자산도 복원, 6번이 테스트·스모크 게이트; 9 run ≈ 160 GB): `square_tent12i_cfix03_s{1,2,3}` = `train.ent_coef=auto_0.3 train.target_ent=12 train.critic_alpha_fixed=0.3`; `square_tent12i_cfix1_s{1,2,3}` = `… train.critic_alpha_fixed=1.0`; `square_tent12i_g099_s{1,2,3}` = `… train.discount=0.99`(코드 변경 없음, 진단용; eval의 mc_return도 γ 0.99 눈금).
  - **Q2 주 지표 = seed-matched `cfix1 − cap1`(s1–3) 127k**(cap1 0.235/0.285/0.200, 평균 0.240; 9/15 배치와의 교차 배치 대비). (a) 평균 차 ≥ +0.15 ∧ 3/3 → **"물림 충격 지지"**(처방 후보: 부드러운 cap 또는 처음부터 물리는 cap); (b) 평균 차 ≤ +0.075(음수 포함) → **"수준 효과(시사)"**(고정 1.0도 cap 1.0보다 낫지 않음 → 1.0은 과함, 상한은 0.3 쪽; n=3 동등성이라 '시사'; 0.075 ≈ Square 127k n=3 SE); (c) 그 외(0.075 < 차 < 0.15, 또는 ≥ 0.15인데 3/3 아님) → 미결. late(102k·127k 평균) 차도 같은 (a)/(b)/(c)로 분류해 함께 보고하고, **127k가 (a)인데 late가 (b), 또는 127k가 (b)인데 late가 (a)면 미결로 강등**. 보조((a)일 때만): cfix1 127k ≥ 0.472(baseline s1–3 = §24 판정표의 '후반 해결' 문턱; 대조군 tent12i 0.485는 그 SE 안)이면 cap1 손실 전부가 물림 충격. **Q1a 의존**: Q1a = 배치 효과면 (a)의 문턱을 +0.15 + |tent12r − tent12 127k 차|로 올리고 (b)는 미결로 강등; Q1a = 미결이면 (b)만 미결로 강등((a) 문턱 그대로); Q1a = 배치 차이 없음이면 그대로.
  - **Q2 sanity = `cfix03 − cap03`(s1–3) late**(102k·127k 평균; cap03 127k 0.430/0.655/0.480은 SD 0.118이라 단일점은 잡음): |평균 차| ≤ 0.075 예상(cap03은 online 10k부터 92% 행에서 물림 → 거의 같은 연산). 어느 결과든 **기록만**(판정 아님); 3/3 ∧ |t| > 4.3이면 cap03의 안 물린 첫 10k가 결과를 바꾼 것으로 적는다.
  - **Q3 γ 진단 = online 110k 5k bin(110,000–114,999)의 actor α seed 평균**(train_log `ent_coef`; 9/16 `diag_bins.csv`의 같은 bin: tent12i 18.2, cap03 1.71, cap1 3.58; 90k bin은 13.7/1.56/3.16): < 3 → "γ가 폭주 원천" 지지; ≥ 10 → 기각; 사이 → 미결. Q_W 예측 tent12i(29.5k)의 ~1/10. 성능(42k·127k)은 목적함수가 달라 **판정하지 않고 기술만**. 지지되면 처방 후보 하나 추가: 보상에는 γ, 엔트로피 보너스에는 더 작은 γ_H(cap보다 원리적일 수 있음; 구현·검정은 별도).
  - 진단 예측: cfix arm `critic_ent_coef` = c 전 행(구성상), actor α는 cap arm과 비슷(cap03 1.6–2.0, cap1 3.6–3.8), Q_W는 cap1 후반 수준(≈4k)에 급변 없이 도달. cfix1의 초반(온도 1.0으로 시작 → 보너스 ≈ 12/step)은 42k에서 cap1 0.343·대조군 0.390과 기술만.
- **검정력·규칙**: Square 127k per-seed SD 0.02–0.12(arm별; §25). 차이 주장: n=3은 §23 규칙(paired df=2, |t| > 4.3 ∧ 3/3), n=5는 df=4 |t| > 2.78 ∧ ≥ 4/5 같은 방향, 그 외 "시사". **동등성 주장**(Q1a 배치 차이 없음, Q2 수준 효과)은 사전 등록한 효과 크기 여백(0.05·0.075 = 해당 대비의 1–1.5 SE)으로 판정하되 n=3이라 항상 "(시사)"를 붙인다. Q2 (a)는 효과 크기 + 3/3 규칙(t 없음)임을 명시. 9/16 keepalive의 'starting' 표시는 `.out` 마지막 500줄에 `[eval]`이 없을 때의 표시(25k 구간에 SB3 표가 500줄을 넘김)였고 프로세스 이상이 아니었다 — 새 노트북은 2,000줄을 본다.
- **쓰면 안 되는 문장**: §25·README §5 그대로. 추가로 "cap1의 실패는 물림 충격/수준 효과"(이 실험이 그 검정 — 결과 전엔 가설), "γ 0.99가 Square 처방"(목적함수 변경; 진단용), "tent12r이 tent12를 재현했다/못 했다"를 위 규칙 없이, "MDAC과 방향 일치".
- **다음 후보(이번엔 안 돌림)**: `square_tent12i_hq_s{1,2,3}`(`train.ent_coef=auto_0.3 train.target_ent=12 train.critic_entropy_scale=0`; hq × α₀ 가산성 검정), `square_prefill_t12i_cap03`, 부드러운 cap(tanh) 또는 γ_H — 1·2 결과 뒤.

## 27. 9/17(목) — §26 결과: Q3(γ)만 지지, Q1a 미결·α₀ 효과 철회, cap 고유 효과(127k) 없음, cfix1 = cap1. 숫자·판정 `results/2026-09-17/README.md`, 채팅용 `HANDOFF_CHAT_2026-09-17.md`

- **실행**: 16 run 전부 정상 종료(9/17 12:38 번들, 170 run). 구성 검증: cfix03/cfix1의 `critic_ent_coef` 전 행 0.300/1.000, tent12r α 최대 21.2–21.9(9/6 tent12 재현). 지표는 `results/2026-09-16/analyze_square_a0_cfix.ps1`(§26 규칙 인코딩, 9/16 번들로 검증)로 계산, 판정은 글자대로.
- **Q1a 미결**: tent12r − tent12(s1–3) 127k +0.078 ± 0.065(2/3; +0.175/−0.045/+0.105), late +0.050 ± 0.047 — 0.05 < |차| < 0.10, 3/3 아님. **동역학은 bin 단위로 재현**(α 0.41 → 20.2, Q_W 144 → 28.7k), **곡선의 중반 하강도 재현**(online 35–70k: tent12 0.33–0.46, tent12r 0.35–0.42); 127k 한 점만 0.397 vs 0.475. 9/6 참조는 그대로 두되 "미결"로 적는다.
- **Q1b**: tent12i − tent12(s1–5) 127k +0.120 ± 0.028(t 4.26, 5/5)·late +0.113 ± 0.014(t 8.06)는 df=4 규칙 통과이나 §26 조항(Q1a ≠ '차이 없음')으로 **'확정' 불가**. 같은 밤 짝 **tent12i − tent12r(s1–3): 127k +0.010 ± 0.039, late +0.048 ± 0.049**(42k +0.083, 62k +0.087, AUC +0.069는 3/3이나 규칙 미달) → **§25의 "α₀ 효과가 tent12 대비 이득의 70%"는 127k 기준으로 철회**, 중반 이득은 시사.
- **Q1c 없음**: cap03 − tent12i(s1–5) 127k +0.028 ± 0.042(2/5), late +0.065 ± 0.032(4/5, t 2.01), 102k +0.101 ± 0.047(4/5, t 2.16). 탐색적(미등록 지표): 62k(env 97,120) **+0.124 ± 0.042(5/5, t 2.93)**; cfix03 − tent12i(교차 배치) 62k +0.093 ± 0.013(3/3, t 7.0), 102k +0.090(3/3, t 4.75), late +0.067 ± 0.015(3/3, t 4.32).
- **Q2 미결(형식)**: cfix1 − cap1(s1–3) 127k −0.017 ± 0.045, late −0.003 ± 0.078 → 둘 다 (b)이나 Q1a 미결로 강등. 기술적으로 **cfix1(처음부터 상수 1.0)은 cap1과 같은 곡선·같은 후반 상태**(α 3.5, Q_W 4.0k; 60–65k부터 붕괴, 127k 0.223 vs 0.240) → 물림 충격은 기제가 아니다. sanity cfix03 − cap03 late −0.010 ± 0.019 → 예상대로(cap03의 안 물린 첫 10k는 무관).
- **Q3 지지**: g099 online 110k bin actor α **1.73**(1.45/1.60/2.12) < 3, Q_W 1,654(tent12i 29.3k의 1/18). γ 0.99이면 cap 없이도 α가 cap03 수준에 머문다 → **α 폭주의 이득은 γ가 정한다**. 성능은 판정 안 함(기술만): 127k 0.333, tent12i 대비 −0.152 ± 0.051(0/3) — γ 0.99는 목적함수 변경이라 처방 아님. g099의 α·Q_W가 cap03과 같은데 성능이 반대 → **α·Q_W 수준은 성능을 정하지 않는다**(§25 기각 재확인: 29k → 0.49, 4k → 0.22, 1.65k → 0.33, 1.35k → 0.52, −75 → 0.49).
- **곡선 관찰(사전 등록 밖, `README` §3)**: 무제한 α arm 셋(tent12·tent12r·tent12i)은 online 25k 고점(0.49–0.57) 뒤 40–70k에서 0.33–0.50으로 내려오고, **critic 온도 0.3 arm(cap03 n=5, cfix03)은 20k부터 끝까지 0.50–0.60 고원**; hq는 중반 0.36–0.44(cap03 − hq late +0.078 3/3 t 8.2; cfix03 − hq +0.068 3/3 t 3.1 → "보너스 0"과 "0.3"은 다르다); 온도 1.0 arm은 60–65k부터 붕괴. 무제한 16 run 62k/127k/late 0.368/0.424/0.402 vs 온도 ≤ 0.3 11 run 0.515/0.513/0.512 vs 1.0 6 run 0.343/0.232/0.274(기술, 배치 혼합). 42k dip은 어느 arm도 못 없앰(0.29–0.40).
- **post-hoc 창 평균 W(사전 등록 아님, README §3b)**: env 72,080–102,128 7점 평균은 arm 내 SD 0.02–0.06 → cap03 W 0.549 ± 0.010(n=5), cfix03 0.539, tent12i 0.469 ± 0.019, hq 0.392, tent12 0.356, tent12r 0.379(tent12r − tent12 −0.004: 9/6 배치가 창에서는 재현); cap03 − tent12i **+0.080 ± 0.025(5/5, t 3.18)**, cfix03 − tent12i +0.076 ± 0.011(3/3, t 6.96) — 사후 선택 창이라 판정이 아니라 다음 사전 등록의 검정력 근거. 맹점: 온도 1.0의 60k 이후 붕괴를 못 봄(cap1 W 0.441 vs 127k 0.240) → W + 127k 둘 다 등록.
- **측정의 한계**: Square 127k 단일점(200 ep)의 arm 내 seed SD 0.03–0.13 → n=3–5 paired SE 0.03–0.07 → §26이 등록한 효과 크기(0.05·0.075·0.10)를 가를 검정력 없음. §25의 127k 기반 주장(α₀ 효과, cap 고유 효과, tent12i ≈ hq)은 전부 "시사" 이하. **다음 실험은 후반 지표를 창 평균(online 40,064–70,096의 7점 × 100 ep)으로 사전 등록**한다.
- **쓰면 안 되는 문장**: `results/2026-09-17/README.md` §5(α₀ 개선·9/6 배치 오류·cap 고유 효과 확인·물림 충격·수준 효과 확정·γ 0.99 처방·Q_W 크면 나쁨·hq = cap03·42k 해결·tent12i ≈ hq·cfix03 교차 배치 확정).
- **다음(권고, 채팅 확정)**: (1) 창 평균 W + 127k를 후반 지표로 사전 등록(post-hoc SE 0.01–0.03 확인됨); (2) **cfix 용량 곡선** `square_tent12i_cfix{01,03,05,1}` × 3(+ cfix03 s4–5) — 물림 시점이 없으므로 이제 해석 가능; (3) γ_H arm(보너스 누적에만 작은 할인; 코드 변경); (4) `square_prefill_t12i_cfix03`. 하지 않는 것: 보상 정규화, β 스케줄, 목표 엔트로피 스케줄, LP-DS, γ 0.99 전면 적용.
