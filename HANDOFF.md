...
> **현재 상태(2026-09-16 낮): §26 — 다음 실험 사전 등록(α₀ 교란 정리 `colab/vm_square_a0.ipynb` 7 run, critic 온도 고정 + γ 진단 `colab/vm_square_cfix.ipynb` 9 run; 코드 `train.critic_alpha_fixed`), 아직 안 돌림. 결과는 §25 + [results/2026-09-16/README.md](results/2026-09-16/README.md)를 먼저 읽을 것 — Square 전이 실험(§24 설계) 결과: 보상 정규화 기각, critic 타깃 보너스 상한 0.3은 α 되먹임을 끊고 Square 무데모 최고(초기 AUC 0.490, 127k 0.522)이나 대조군 `square_tent12i`(초기 α 0.3, 127k 0.485)가 tent12 대비 이득의 대부분을 설명 → cap 고유 효과는 시사; cap 1.0은 해로움; `can_prefill_fixa015` 129k 0.930. 채팅용 요약 [HANDOFF_CHAT_2026-09-16.md](HANDOFF_CHAT_2026-09-16.md). 문헌 §24.1.**
