...
4. MDAC (Iwaki, 2025, arXiv:2502.03854) — 타깃 내 α log π 항 유계화 선례이자 진단 일치: "느리게 포화하는(거의 활성화되지 않는) bound가 자주 물리는 bound보다 낫다"(Fig. 4c–d; 9/16 원문 확인: '공격적' = clip(x,−1,1)처럼 클리핑이 잦은 bound, 값의 크기가 아님)는 결과는 우리 cap03(92% 물림) > cap1(79%)과 방향이 맞지 않으므로 cap 값 선택의 근거로 인용하지 않고, §26 '물림 충격' 가설(부드러운/이른 bound)과의 접점으로만 언급; Munchausen RL (NeurIPS 2020)은 "타깃 내 log-policy 항은 유계화해야 한다"는 동기 인용으로만 사용하고 통제군으로는 쓰지 않음.