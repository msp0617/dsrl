# 인수인계 (채팅 AI용: Gemini / claude.ai) — DSRL offline-to-online dip 연구, 2026-09-06 17:10 KST 기준
...
**지금**: 82 run 완료, 8 run 진행 중(17:40·18:30 완료 예정). Colab 크레딧 약 25(19:18에 소진), 충전은 내일. 이후 실험은 Google Cloud 무료 체험판(₩415k)으로 이사 예정(§10).
...
| mix_prefill (논문 기본: 데모를 리플레이에, 비율 0.91→0.43 자연 감쇠) | 3 | 0.09 @34k | 47k | **0.68** | **0.84** | **0.86** | 0.89 |
| mix_fixed 0.5 (RLPD식 대칭 샘플링) | 3 | 0.29 | 44k | 0.66 | — | 0.84 | 0.85 |
...
| 고정 0.1 (엔트로피 3~8) | 3 | 0.14 | 0.51 | 0.63 | 0.62 |
| **고정 0.3** (엔트로피 10~14) | **5** | **0.47** (기준선 위, dip 없음) | **0.66** | **0.79** | **0.74** |
| 고정 1.0 (엔트로피 ~17) | 3 | 0.03 | 0.25 | 0.26 | 0.15 |
...
| can_tent6 (auto-α, 목표 6, 100k) | 3 | **진행 중, 18:30** | | | |

### 2.3 Square (rollout 32k, 150k, 마지막 평가 127k)
| 조건 | n | 37k / 42k / 47k / 52k | AUC 37~100k | 100k | 127k (seed별) |
|---|---|---|---|---|---|
...
| square_fixalpha_03 (고정 0.3) | **5** | 0.33 / 0.37 / 0.34 / 0.44 (n=5 근사) | 0.42 | ~0.40 | **0.37** (0.09/0.32/0.60/0.52/0.30) |
| square_tent12 (목표 12) | 3 | 0.34 / **0.41** / 0.46 / 0.51 | 0.43 | 0.34 | 0.40 (0.34/0.46/0.39) |
| square_tent6 (목표 6) | 3 | 0.31 / 0.28 / 0.34 / 0.44 | ~0.40 | ~0.30 | **0.29** (0.39/0.20/0.29), 87k 이후 3 seed 모두 0.2~0.3 |
(fixalpha_03 s4·s5와 tent6은 17:40에 끝나지만 마지막 평가 127k는 이미 기록됨. 위 표의 s4·s5·tent6 초반 값은 seed별 원시값 평균.)
...
3. **엔트로피 바닥이 dip 깊이를 정한다 — 두 과제에서 같은 용량-반응.**
   | 과도기 엔트로피 바닥 | Can 34k | Square 42k |
   |---|---|---|
   | 0 (auto-α, 목표 0) | 0.24 | 0.21 |
   | ~5 (Can 고정 0.1 / square_tent6) | 0.14 | 0.28 |
   | ~11 (Can 고정 0.3, tent12i / square_tent12) | **0.47** | **0.41** |
   (Can 고정 0.1은 엔트로피가 3~8로 흔들려 대리값. can_tent6이 오늘 저녁 이 칸을 같은 메커니즘으로 채운다.)
4. **Can에서는 처방이 성립한다.** 고정 0.3 또는 목표 12 + 초기 0.3이면 dip이 없고 100k 0.68~0.79(baseline 0.48), 데모 없이 데모 넣은 것과 같은 AUC.
5. **Square에서는 dip 제거와 최종 성능이 상충한다.** 목표 12는 dip을 없애지만 127k 0.40, 목표 6은 dip 절반에 127k 0.29, 둘 다 baseline 0.47 아래. 고정 0.3은 5 seed 전부 엔트로피가 0 아래로 무너지고(logp +1.7~+4.8, |mu| 0.83~0.90) 127k 0.37(편차 큼). **이유(가설, 진단 그림으로 확인 예정)**: Square는 Q 스케일이 Can의 5배이고 학습 내내 커진다(Q_W 300~700). 엔트로피를 고정 목표에 붙들면 α가 1 → 15까지 계속 올라가야 하고, 그 α가 critic 타깃의 엔트로피 보너스(α·log π')에도 들어가 Q_W를 부풀려 후반이 불안정해진다. auto-α(목표 0)는 α 0.2~0.3에 머물러 그 문제가 없는 대신 dip을 겪는다.
6. **결론 문장**: "DSRL의 O2O dip은 SAC auto-α가 목표 엔트로피 0을 향해 정책을 무너뜨리는 15k 창이다. 초기에 엔트로피를 지키면 dip은 두 과제에서 사라진다. 그러나 Q 스케일이 커지는 과제에서는 고정 목표가 후반 성능을 깎으므로, 처방은 '초기엔 지키고 뒤에 낮추는' 목표 엔트로피 스케줄이며 언제 낮출지가 다음 질문이다."
...
## 6. 지금 상태 (9/6 17:10)
- 82 run 완료. 진행 중(Colab G4 1대, 자동 반납 keepalive): `square_fixalpha_03_s{4,5}`, `square_tent6_s{1,2,3}`(150k, **17:40** 완료), `can_tent6_s{1,2,3}`(100k, **18:30** 완료 예상). 크레딧은 19:18에 소진되므로 그 전에 끝나야 함. 안 끝나면 25k 체크포인트에서 내일 resume.
- CSV 묶음: 로컬 `Downloads\csv_bundle (4).zip`(오늘 12:45까지). 그림·`metrics.csv`: `Downloads\dsrl_figs_0906\`.
- 결과는 Drive `dsrl_project/logs/<exp_id>/{eval_log.csv,train_log.csv}`, `<exp_id>.out`.

## 7. 오늘 저녁 순서
1. 17:40: keepalive 정지 → zip 셀(아래) → 다운로드 → Claude Code에 경로 → keepalive 재실행. 18:30 Can 끝나면 한 번 더(크레딧 0이면 CPU 런타임으로).
   ```bash
   %%bash
   PROJ=/content/drive/MyDrive/dsrl_project
   for E in square_fixalpha_03_s4 square_fixalpha_03_s5 square_tent6_s1 square_tent6_s2 square_tent6_s3 can_tent6_s1 can_tent6_s2 can_tent6_s3; do
   ...
   ```
2. 로컬 분석(Claude Code): Square 축에 tent6, 적응형 축에 can_tent6 추가, 진단 그림으로 §3.5 가설(α 상승 → Q_W 부풀림) 확인, 용량-반응 표 완성.
3. 포스터 6패널 문안 확정(§8). 월요일 아침 결과 반영은 6번 패널 한 줄.
...
6. **Square: 같은 용량-반응, 그러나 상충**. 42k 바닥 재현(예측 적중). 엔트로피 바닥 0/5/11 → dip 0.21/0.28/0.41. 하지만 엔트로피를 붙들면 후반을 잃음(127k 0.29/0.40 vs 0.47), 고정 0.3은 엔트로피가 다시 무너짐(5/5 seed). 결론: "초기엔 지키고 뒤에 낮추는 스케줄, 언제 낮출지가 다음 질문". LP-DS 인용. `success_square.png`.
...
- §3.5의 "α 상승이 critic 타깃을 부풀린다"는 진단 그림 확인 전까지 **가설**.
...
- 후속 실험 후보: 목표 엔트로피 스케줄(초기 12 → 후반 0, 낮추는 시점 = 원래 게이트의 센서 질문), Lift·Transport 추가, seed 확장, Square에서 α 상승과 critic 타깃 결합 분리(`critic_entropy_scale`로 타깃의 보너스만 끄고 목표 엔트로피 유지).
